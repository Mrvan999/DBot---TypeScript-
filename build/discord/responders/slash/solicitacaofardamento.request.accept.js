...
import { uniformFinalButtons } from "../../buttons/commands/slash/public/solicitacaofardamento.final.js";
createResponder({
    customId: "uniform/accept/:memberId",
    ...
    async run(interaction, { memberId }) {
        if (!interaction.member.roles.cache.has(dbroles.dl_roles.diretoriadelogisticaRoleId)) {
            await interaction.reply({
                flags: ["Ephemeral"],
                content: `${icon.action_x} Você não possui permissão para aceitar personalizações.`
            });
            return;
        }
        ...
        await docRef.update({
            status: `Em personalização desde: ${time(new Date(), TimestampStyles.LongDateShortTime)}`
        });
        ...
        messagemd = messagemd
            ...
            .replace(/\$\{statusAtual\}/g, `Em personalização desde: ${time(new Date(), TimestampStyles.LongDateShortTime)} | <@${interaction.user.id}>`);
        await interaction.update({
            content: messagemd,
            components: [uniformFinalButtons(solicitante).toJSON()]
        });
        try {
            await solicitante.send({
                content: brBuilder(`Olá, ${solicitante.user.username}. Seu fardamento foi aceitado pela Diretoria de Logística e será entregue em breve.`, `Data da aceitação: ${time(new Date(), TimestampStyles.LongDateShortTime)}`, `Responsável: <@${interaction.user.id}>`)
            });
        }
        catch (error) {
