    ...
    async run(interaction, { memberId }) {
        ...
        try {
            await solicitante.send({
                content: brBuilder(`Olá, ${solicitante.user.username}. Seu fardamento foi negado pela Diretoria de Logística, entre em contato para mais informações.`, `Data da reprovação: ${time(new Date(), TimestampStyles.LongDateShortTime)}`, `Responsável: <@${interaction.user.id}>`)
            });
        }
        catch (error) {
