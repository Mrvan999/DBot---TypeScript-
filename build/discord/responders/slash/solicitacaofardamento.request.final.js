...
createResponder({
    customId: "uniform/final/:memberId",
    ...
    async run(interaction, { memberId }) {
        ...
        await docRef.update({
            status: `Finalizado desde: ${time(new Date(), TimestampStyles.LongDateShortTime)}`
        });
        ...
        messagemd = messagemd
            ...
            .replace(/\$\{statusAtual\}/g, `Finalizado desde: ${time(new Date(), TimestampStyles.LongDateShortTime)} | <@${interaction.user.id}>`);
        ...
        try {
            await solicitante.send({
                content: brBuilder(`Olá, ${solicitante.user.username}. Seu fardamento foi finalizado pela Diretoria de Logística e será entregue em até 1 (uma) hora.`, `Data da finalização: ${time(new Date(), TimestampStyles.LongDateShortTime)}`, `Responsável: <@${interaction.user.id}>`)
            });
        }
        catch (error) {
