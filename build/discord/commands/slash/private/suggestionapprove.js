...
import { reproveSuggestionContainer } from "../../../containers/events/suggetions.reprove.js";
createCommand({
    name: "sugestão",
    ...
    options: [
        {
            name: "aprovar",
            description: "Aprova uma sugestão - Admin",
        ...
        {
            name: "reprovar",
            description: "Reprova uma sugestão - Admin",
            type: ApplicationCommandOptionType.Subcommand,
            options: [
                {
                    name: "identificador",
    ...
    async run(interaction) {
        if (interaction.options.getSubcommand() === "aprovar") {
        ...
        else if (interaction.options.getSubcommand() === "reprovar") {
            const identificador = interaction.options.getString("identificador");
            const motivo = interaction.options.getString("motivo") ?? "Não há";
            await interaction.deferReply({
                flags: ["Ephemeral"]
            ...
            const downcount = (downReaction?.count ?? 1) - 1;
            await interaction.editReply({
                content: `${icon.action_check} Sugestão reprovada.`
            });
            await suggestionMessage.edit({
                flags: ["IsComponentsV2"],
                components: [reproveSuggestionContainer(data.sugestao, responsavelSuggestion, interaction.member, motivo, String(upcount), String(downcount), identificador)]
            });
            await suggestionMessage.reactions.removeAll();
            await suggestionMessage.thread.setLocked(true);
            await suggestionMessage.thread.setArchived(true);
