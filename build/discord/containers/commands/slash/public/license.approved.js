...
export async function licenseApprovedContainer(memberDp, member, motivo, tempo, observacoes) {
    return createContainer({
        components: [
            createSection(brBuilder(`## ${icon.clock_check} Licença Aprovada`, `${icon.clock} ${time(new Date(), TimestampStyles.LongDateShortTime)}`, `${icon.user} ${member.user} (${member.id})`), {
                media: {
