    ...
    async run(interaction, { memberId }) {
        ...
        try {
            await solicitante.send({
                content: brBuilder(
                    `Olá, ${solicitante.user.username}. Seu fardamento foi aceitado pela Diretoria de Logística e será entregue em breve.`,
                    `Data da aceitação: ${time(new Date(), TimestampStyles.LongDateShortTime)}`,
