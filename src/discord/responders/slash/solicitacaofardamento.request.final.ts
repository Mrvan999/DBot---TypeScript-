    ...
    async run(interaction, { memberId }) {
        ...
        try {
            await solicitante.send({
                content: brBuilder(
                    `Olá, ${solicitante.user.username}. Seu fardamento foi finalizado pela Diretoria de Logística e será entregue em até 1 (uma) hora.`,
                    `Data da finalização: ${time(new Date(), TimestampStyles.LongDateShortTime)}`,
